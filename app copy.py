...
with tab1:
    ...
    if img_file is not None:
        ...
        # --- FORCE a clean remount exactly once per new image ---
        if st.session_state.get("last_img_hash") != img_hash:
            st.session_state["last_img_hash"] = img_hash
            st.session_state["force_canvas_remount"] = st.session_state.get("force_canvas_remount", 0) + 1
            st.rerun()

        canvas_key = f"canvas_{img_hash}_{st.session_state.get('force_canvas_remount', 0)}"
